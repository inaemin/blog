      ...
      {comments.map((comment, index) => (
        <article key={comment.id} className="flex flex-col gap-2 rounded-lg border border-border bg-card p-3.5 xl:p-4">
          <div className="flex h-7.5 items-center gap-2.5 md:h-8 xl:h-9">
            <span className={`flex size-7.5 items-center justify-center rounded-full text-[11px] font-medium leading-[1.35] text-white md:size-8 xl:size-8.5 xl:text-xs ${avatarColorClassNames[index % avatarColorClassNames.length]}`}>
          ...
          <p className="text-[13px] leading-4.5 text-text-strong xl:text-sm xl:leading-4.75">{comment.body}</p>
        </article>
      ))}
    </div>
  );
